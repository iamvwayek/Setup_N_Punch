...
function ScaleCard({ topVal }) {
  ...
  return (
    <div className="absolute bg-black/70 w-full h-[100vh] z-100" style={{ top: topVal }} onClick={handleClickFn}>
      <div className="w-full h-full flex flex-col p-5 justify-center items-center text-4xl text-white font-card">
        <div className=" flex flex-col gap-4 mb-4 select-none">
          <h1 className=" font-semibold">{scaleJoke.setup}</h1>
          <p className="font-extralight">{scaleJoke.punch}</p>
          <button className="w-full flex items-center justify-end gap-3">
            <div id="tip-tool" className=" absolute mr-8 px-2 font-light text-lg bg-black/70 w-25 h-7 rounded-2xl hidden transition-all ease-in-out duration-200">{text}</div>
